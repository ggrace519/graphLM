...
- `--max-output-tokens` CLI flag / `GRAPHLM_MAX_OUTPUT_TOKENS` env var to configure the graph output-token ceiling (default raised from 16000 to 32000, sized from measurement). This value is both the `max_tokens` requested and the pass-2 output reserve, kept in lock-step; raise it if a very large project's graph still truncates (#18)
